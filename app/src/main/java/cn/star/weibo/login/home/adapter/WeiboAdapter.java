...
import cn.star.weibo.login.home.widget.NewPauseOnScrollListener;
import cn.star.weibo.mvp.presenter.WeiBoArrowPresent;
import cn.star.weibo.mvp.presenter.iml.WeiBoArrowPresenterImp;
import cn.star.weibo.view.widget.EmojiTextView;

/**
 * Created by dcl on 2015/12/29.
 */
public abstract class WeiboAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ORINGIN_ITEM) {
            ...
            mView = LayoutInflater.from(mContext).inflate(R.layout.home_weiboitem_retweet_pictext, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        if (holder instanceof OriginViewHolder) {
            ...
            ((RetweetViewHolder) holder).retweet_weibo_layout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(mContext, RetweetDetailActivity.class);
                    intent.putExtra("weiboitem", mDatas.get(position));
                    mContext.startActivity(intent);
                }
            });
    ...
    public static class RetweetViewHolder extends ViewHolder {
        ...
        public RetweetViewHolder(View v) {
            ...
            popover_arrow = (ImageView) v.findViewById(R.id.popover_arrow);
            profile_name = (TextView) v.findViewById(R.id.mine_user_name);
            profile_time = (TextView) v.findViewById(R.id.mine_user_detail_time);
            retweet_content = (EmojiTextView) v.findViewById(R.id.retweet_content);
            weibo_comefrom = (TextView) v.findViewById(R.id.mine_user_detail_from);
            redirect = (TextView) v.findViewById(R.id.redirect);
            comment = (TextView) v.findViewById(R.id.comment);
            feedlike = (TextView) v.findViewById(R.id.feedlike);
