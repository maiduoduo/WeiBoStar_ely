...
import cn.star.weibo.api.StatusesAPI;
...
public class StatusListModelImp implements StatusListModel {
    ...
    public boolean cacheLoad(long groupType, Context context, OnDataFinishedListener onDataFinishedListener) {
        ...
        if (groupType == Constants.GROUP_TYPE_ALL) {
            response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + "/weibo/home", "全部微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
        } else if (groupType == Constants.GROUP_TYPE_FRIENDS_CIRCLE) {
            response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + "/weibo/home", "好友圈" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
        } else {
            response = SDCardUtil.get(context, SDCardUtil.getSDCardPath() + "/weibo/home", groupType + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt");
        }
        if (response != null) {
    ...
    public void cacheSave(long groupType, Context context, StatusList statusList) {
        ...
        if (groupType == Constants.GROUP_TYPE_ALL) {
            SDCardUtil.put(context, SDCardUtil.getSDCardPath() + "/weibo/home", "全部微博" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
        } else if (groupType == Constants.GROUP_TYPE_FRIENDS_CIRCLE) {
            SDCardUtil.put(context, SDCardUtil.getSDCardPath() + "/weibo/home", "好友圈" + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
        } else {
            SDCardUtil.put(context, SDCardUtil.getSDCardPath() + "/weibo/home", groupType + AccessTokenKeeper.readAccessToken(context).getUid() + ".txt", response);
        }
    }
